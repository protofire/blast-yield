...
export const encodeGetYieldMode = (
  ...
  return {
    to: token.address,
    value: '0',
    data: yieldContractInterface.encodeFunctionData(functionName, [
      contractAddress,
    ]),
  };
};
...
export const encodeClaimYield = (
  ...
  const functionName =
    token.type === TokenType.NATIVE_TOKEN ? 'claimYield' : 'claim';
  ...
  const parsedAmount =
    safeParseUnits(amount, token.decimals)?.toString() || '0';
